...
#Courses related viewwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww
@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
def admin_course_view(request):
    return render(request,'school/admin_course.html')


@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
def admin_take_course_view(request,cl):
    students=models.StudentExtra.objects.all().filter(cl=cl)
    print(students)
    aform=forms.CourseForm()
    if request.method=='POST':
        form=forms.CourseForm(request.POST)
        if form.is_valid():
            Course=request.POST.getlist('present_status')
            date=form.cleaned_data['date']
            for i in range(len(Course)):
                CourseModel=models.Course()
                CourseModel.cl=cl
                CourseModel.date=date
                CourseModel.present_status=Course[i]
                CourseModel.roll=students[i].roll
                CourseModel.save()
            return redirect('admin-course')
        else:
            print('form invalid')
    return render(request,'school/admin_take_course.html',{'students':students,'aform':aform})



@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
def admin_view_course_view(request,cl):
    form=forms.AskDateForm()
    if request.method=='POST':
        form=forms.AskDateForm(request.POST)
        if form.is_valid():
            date=form.cleaned_data['date']
            coursedata=models.Courses.objects.all().filter(date=date,cl=cl)
            studentdata=models.StudentExtra.objects.all().filter(cl=cl)
            mylist=zip(coursedata,studentdata)
            return render(request,'school/admin_view_course_page.html',{'cl':cl,'mylist':mylist,'date':date})
        else:
            print('form invalid')
    return render(request,'school/admin_view_course_ask_date.html',{'cl':cl,'form':form})
...
def course_dashboard_view(request):
    ...
    #     'notice':notice
    # }
    return render(request,'school/course_dashboard.html',context=mydict)
